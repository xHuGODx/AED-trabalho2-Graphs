...
Graph* GraphCopy(const Graph* g) {
  ...
  List* vertices = g->verticesList;
  List* newVertices = newGraph->verticesList;

  ListMoveToHead(vertices);
  ListMoveToHead(newVertices);
  for (unsigned int i = 0; i < ListGetSize(vertices); i++){
    struct _Vertex* cur = ListGetCurrentItem(vertices);
    struct _Vertex* new_cur = ListGetCurrentItem(newVertices);
    new_cur->id = cur->id;
    ListMoveToNext(vertices);
    ListMoveToNext(newVertices);
  }

  ListMoveToHead(vertices);
  for (unsigned int i = 0; i < ListGetSize(vertices); i++){
    struct _Vertex* cur = ListGetCurrentItem(vertices);
    List* edges = cur->edgesList;
    ListMoveToHead(edges);
    for (unsigned int i = 0; i < ListGetSize(edges); i++){
      struct _Edge* adj = ListGetCurrentItem(edges);
      (g->isWeighted) ? GraphAddWeightedEdge(newGraph, cur->id, adj->adjVertex, adj->weight) : GraphAddEdge(newGraph, cur->id, adj->adjVertex);
      ListMoveToNext(edges);
    }
    ListMoveToNext(vertices);
  }
  assert(GraphCheckInvariants(newGraph) != 0);
  return newGraph;
}
...
Graph* GraphFromFile(FILE* f) {
  ...
  for (int i = 0; i < 4; i++){
    ...
    graphInfo[i] = atoi(line);
  }

  Graph* g = GraphCreate(graphInfo[2], graphInfo[0], graphInfo[1]);
  int v1, v2;
  double w;

  while (fgets(line, sizeof(line), f) != NULL) {
    if (sscanf(line, "%d %d %lf", &v1, &v2, &w) >= 2){
      if (v1 != v2){
        if (graphInfo[1]){
          GraphAddWeightedEdge(g, v1, v2, w * 100);
        }
        else GraphAddEdge(g, v1, v2);
      }
    }  
  }
...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  assert(g != NULL);
  ListMove(g->verticesList, v);
  struct _Vertex* vertex_i = ListGetCurrentItem(g->verticesList);
  ListMove(g->verticesList, w);
  struct _Vertex* vertex_e = ListGetCurrentItem(g->verticesList);
  ListMoveToHead(vertex_i->edgesList);
  for (unsigned int i = 0; i < ListGetSize(vertex_i->edgesList); i++){
    struct _Edge* edge = ListGetCurrentItem(vertex_i->edgesList);
    if (edge->adjVertex == w){
      DEGREEUPDT+=2;
      DAGCESS+=2;
      free(edge);
      ListRemoveCurrent(vertex_i->edgesList);
      vertex_i->outDegree--;
      vertex_e->inDegree--;
      g->numEdges--;
      if (!g->isDigraph) {
        ListMoveToHead(vertex_e->edgesList);
        for (unsigned int e = 0; e < ListGetSize(vertex_e->edgesList); e++){
          struct _Edge* edge = ListGetCurrentItem(vertex_e->edgesList);
          if (edge->adjVertex == v){
            DEGREEUPDT+=2;
            DAGCESS+=2;
            ListRemoveCurrent(vertex_e->edgesList);
            free(edge);
            vertex_e->outDegree--;
            vertex_i->inDegree--;
            break;
            ...
            ListMoveToNext(vertex_e->edgesList);
          }
        }
      }
      break;     
      ...
      ListMoveToNext(vertex_i->edgesList);
    }
  }
  return 0;
}
...
int GraphCheckInvariants(const Graph* g) {
  assert(g != NULL);
  ////////////////////
  if (GraphGetNumVertices(g) != ListGetSize(g->verticesList)) return 0;
  ////////////////////
  if (GraphGetNumEdges(g) > g->numVertices * (g->numVertices - 1) / 2) return 0;
  //////////////////// only for non-digraphs
  if (!g->isDigraph){
    unsigned int count = 0;
    unsigned int degree = 0;
    ListMoveToHead(g->verticesList);
    for (unsigned int i = 0; i < g->numVertices; i++){
      degree = GraphGetVertexDegree(g, i);
      if (degree > g->numVertices - 1) return 0;
      struct _Vertex* ver = ListGetCurrentItem(g->verticesList);
      if (degree != ListGetSize(ver->edgesList)) return 0;
      count += GraphGetVertexDegree(g, i);
      ListMoveToNext(g->verticesList);
    }
    if (2 * GraphGetNumEdges(g) != count) return 0;
  }
  ////////////////////
  ////////////////////
  if (!g->isDigraph) return 1;
  //////////////////// from now on only digraph properties
  unsigned int countOUT = 0;
  unsigned int countIN = 0;
  for (unsigned int i = 0; i < g->numVertices; i++){
    countOUT += GraphGetVertexOutDegree(g, i);
    countIN += GraphGetVertexInDegree(g, i);
  }
  if ((countOUT != countIN) || (countOUT != g->numEdges)) return 0;
